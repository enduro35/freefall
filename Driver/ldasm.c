...
unsigned long __fastcall SizeOfCode(void *Code, unsigned char **pOpcode)
{
	...
	} else 
	{
		...
		/*äëÿ îïêîäî?A0 - A3 ïðåôôèêñ 66 èíâåðòèðóåòñÿ*/
		if (Opcode >= 0xA0 && Opcode <= 0xA3) PFX66 = PFX67;
	}
	cPtr++;
	if (Flags & OP_WORD) cPtr++;	
	/*îáðàáàòûâàåì MOD r/m*/
	if (Flags & OP_MODRM)
	{
		iMod = *cPtr >> 6;
		iReg = (*cPtr & 0x38) >> 3;  
...
/*Ïîëó÷åíè?ðàçìåð?ôóíêöè?ïî óêàçàòåë íà íå?(ðàçìåð äî ïåðâîé êîììàíäû RET)*/
...
/*îïðåäåëåíè?òîãî, èìåå?ëè êîììàíäà rel32 offset*/
